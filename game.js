...
setupTerrain() {
    ...
    const seed = "9877766666677789877665555556677877665544445566777665444334445667765443333334456765543322223345566544321111234456654332100123345665433210012334566544321111234456655433222233455676544333333445677665444334445667787766555555667789877766666677789";
    ...
    setTimeout(() => overlay.remove(), 3000);
        ...
        class TerrainSystem {
            constructor(gridSize, tileSize, heightLevels) {
                ...
                let min = 1, max = 3.33;
 this.heightScales = Array.from({ length: this.heightLevels }, (_, h) =>
   min + (h/(this.heightLevels-1))*(max-min)
);
...
// Add this method to TerrainSystem class
generateSeed() {
    let seed = '';
...
loadFromSeed(seed) {
    ...
    return true;
            ...
            generateTerrain() {
                ...
                if (window.game && window.game.debugSystem) window.game.debugSystem.updateSeedDisplay();
            }
...
create3DTile(x, z, height) {
  ...
  // position so that bottom sits on y=0 (half of h) plus a tiny cushion if you like:
  ...
  raycastPlane.material.depthTest  = false;
   ...
   if (newMesh.children) {
    newMesh.children.forEach(child => {
      // only shadow the visible art; skip the invisible raycast plane
