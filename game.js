...
setupIntro() {
    ...
    setupRenderer() {
        ...
        this.renderer.setClearColor(0x4FB3D9);
        this.renderer.shadowMap.enabled = true;
        this.renderer.shadowMap.type = THREE.PCFSoftShadowMap;
    }

    // Delegate camera methods to camera system
    setCameraPreset(presetName, smooth = true, callback = null) {
        ...
        this.cameraSystem.setCameraPreset(presetName, smooth, callback);
    }
    ...
    setupLighting() {
        ...
        Object.assign(this.directionalLight.shadow.camera, {
            near: 0.1, far: 200, left: -50, right: 50, top: 50, bottom: -50
        });
    ...
    setupInput() {
        ...
        ['keydown', 'keyup'].forEach(evt => 
            addEventListener(evt, e => this.input.keys[e.code] = evt === 'keydown')
        );
        
        // Mouse events
        ['mousedown', 'mousemove', 'mouseup'].forEach(evt => 
            this.canvas.addEventListener(evt, e => this.handleMouse(evt, e))
        );
        
        this.canvas.addEventListener('click', e => this.editMode && this.handleTileClick(e));
        this.canvas.addEventListener('wheel', e => {
            e.preventDefault();
            this.cameraSystem.zoomCamera(e.deltaY > 0 ? 0.1 : -0.1);
        });
    ...
    setupUI() {
        ...
        document.getElementById('clearBtn').onclick = () => 
            confirm('Clear all terrain?') && this.clearTerrain();
    ...
    handleMouse(evt, e) {
        ...
        if (evt === 'mousedown') {
            ...
            m.x = e.clientX;
            m.y = e.clientY;
        } else if (evt === 'mousemove' && m.pressed && !this.editMode) {
            this.cameraSystem.panCamera(e.clientX - m.x, e.clientY - m.y);
        } else if (evt === 'mouseup') {
            m.pressed = false;
        }
    ...
    highlightTile(mesh) {
        ...
        if (USE_SPRITE_TERRAIN && mesh.isGroup) {
            ...
            this.tileHighlight = new THREE.LineSegments(edges, 
    ...
    modifySelected(delta) {
        this.selectedTile && this.modifyTileHeight(this.selectedTile.x, this.selectedTile.z, delta);
    }
    ...
    updateEditUI() {
        const elems = ['selectedTile', 'selectedHeight', 'raiseBtn', 'lowerBtn']
        ...
        if (this.selectedTile) {
            ...
            elems[0].textContent = 'None';
            elems[1].textContent = '-';
            elems.slice(2).forEach(btn => btn.disabled = true);
        }
    }

    // Touch handling methods (simplified)
    handleTouchStart(e) {
        const touches = e.touches;
        if (touches.length === 1) {
            Object.assign(this.input.touch, {
                active: true, moved: false,
                start: { x: touches[0].clientX, y: touches[0].clientY }
            });
        } else if (touches.length === 2) {
            this.input.touch.pinchDistance = Math.hypot(
    ...
    handleTouchMove(e) {
        ...
        if (touches.length === 1 && this.input.touch.active) {
            const delta = {
                x: (touches[0].clientX - this.input.touch.start.x) / window.innerWidth,
                y: (touches[0].clientY - this.input.touch.start.y) / window.innerHeight
            };
            const distance = Math.hypot(delta.x, delta.y);
            if (distance > 0 && !this.editMode) {
                this.input.touch.moved = true;
                this.cameraSystem.panCamera(delta.x * 70, delta.y * 70);
            }
        } else if (touches.length === 2) {
            ...
            this.cameraSystem.zoomCamera((this.input.touch.pinchDistance - currentDist) * 0.01);
    ...
    handleKeyboardInput() {
        // Delegate camera and lighting controls to camera system
        this.cameraSystem.handleKeyboardCameraInput();
    ...
    update() {
        ...
        // Update player and proximity interactions
    ...
    // Interaction system methods (keeping these as they're game-specific)
    setupInteractables() {
        ...
        interactableData.forEach(data => {
            ...
            if (obj) {
                ...
                this.interactables.push({
                    mesh: obj, x: data.x, z: data.z, type: data.type,
                    message: data.message, interact: data.interact, inRange: false
                });
            }
        });
        
        // Setup NPCs after a delay
        setTimeout(() => {
            if (this.npcs) {
                this.npcs.forEach(npc => {
                    if (npc.isInteractable) {
                        this.interactables.push({
                            mesh: npc.sprite, x: npc.pos.x, z: npc.pos.z, type: 'npc',
                            message: npc.name || 'Friendly NPC', interact: 'Press E to talk',
                            inRange: false, npcRef: npc
                        });
                    }
                });
            }
        ...
        switch (type) {
            ...
            case 'crystal':
                ...
                material = new THREE.MeshLambertMaterial({ 
                    color: 0x00FFFF, transparent: true, opacity: 0.8, emissive: 0x004444 
                });
                obj = new THREE.Mesh(geometry, material);
            ...
            case 'shrine':
                ...
                const topMesh = new THREE.Mesh(top, new THREE.MeshLambertMaterial({ 
                    color: 0xFFD700, emissive: 0x332200 
                }));
